...
import { Upload, X, ArrowLeft, ArrowRight, Image as ImageIcon, Sparkles } from 'lucide-react';
...
export function ImageUploader({ images, onChangeImages }: ImageUploaderProps) {
    ...
    return new Promise((resolve, reject) => {
      ...
      reader.onload = (event) => {
        ...
        img.onload = () => {
          ...
          const maxDimension = 1200; // Resize large images to max 1200px
          let width = img.width;
  ...
  const uploadFile = async (file: File) => {
    try {
      ...
      toast.error(err.message || 'Image upload failed. Is your product-images bucket created?');
    } finally {
        ...
        <div className="flex flex-col items-center justify-center space-y-2">
          ...
          <div>
            <p className="text-sm font-semibold text-gray-700">
              Drag & drop image here, or <span className="text-emerald-600">browse file</span>
            </p>
            <p className="text-xs text-gray-400 mt-1 flex items-center justify-center gap-1">
              <Sparkles className="w-3 h-3 text-amber-500" /> Auto compress & resize to max 1200px
            </p>
          </div>
        </div>
